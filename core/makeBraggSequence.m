...
if numel(T) == 1
    T = repmat(T,numPulses-1,1);
elseif numel(T) > (numPulses - 1)
    error('Number of time intervals must be 1 less than the number of pulses!');
end
...
%     idx = (t > (nn-1)*t0) & (t < ((nn-1)*t0 + (nn/2-1)*T));
    idx = (t - t0) > (sum(T(1:nn)) + sum(T(1:(nn-1))))/2;
