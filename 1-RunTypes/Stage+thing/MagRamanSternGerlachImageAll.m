...
%     sq.find('WG AMP 3').after(tDipoleRamp,sq.linramp(tDipoleRamp,0.0,2));
    ...
    sq.find('87 Cooling Freq EOM').after(0.2e-3,Freq2ToV(0,-60,'c')); %cooling normally 0 detuning
	...
	TwoPhotonDetuning = 0.5e5+6.834682611e9+0.02e5;
    ...
    %pulse parameters   
    pulseAmpch2 = 0;
    pulseAmpch1 = RamanOnOff*5000; %5000
...
% 	pulseWidth = varargin{1}; 
    ...
    %pulse
    RamanDDS1Frequency = (110+0*freq2hk)*ones(size(tBragg));
    ...
    RamanDDS2Frequency = 110.-0-dopplerBragg*0*(timeSinceDrop+tBragg); %compensate for Doppler
    ...
    %%Stern-Gerlack Pulse
    sq.anchor(sq.latest);
	sq.find('3D Coils Bottom').set(1);
    sq.find('3D Coils Top').set(1);
    OnOff = 1;
    RamanOnOff = 0;
	sq.find('3D Coils Bottom TTL').set(OnOff);
    sq.find('3D Coils Top TTL').set(OnOff);
    
    TStern = 5e-3;
	sq.find('3D Coils Bottom').after(TStern,0);
    sq.find('3D Coils Top').after(TStern,0);
	sq.find('3D Coils Bottom TTL').after(TStern,0);
    sq.find('3D Coils Top TTL').after(TStern,0);
