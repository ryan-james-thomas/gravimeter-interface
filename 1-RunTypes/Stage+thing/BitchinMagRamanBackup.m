...
%     
  ...
  %  RStable(:,end+1) = [varargin{2}+6.834682611e9; 19];
    RStable(:,end+1) = [0.5e5+6.834682611e9; 19];
%     RStable(:,end+1) = [36000.6+6.834682611e9; 19];
    ...
    sq.find('RS Microwave TTL').after(100e-6,0);   
    tBragg= linspace(tBraggPulse,tBraggPulse+800e-6,400);
    braggAmpch2 = 0;
    braggAmpch1 = 3000;
    braggWidth = 20e-6; 
    ...
    sq.dds(1).after(tBragg,(110+0*freq2hk)*ones(size(tBragg)),sq.gauspulse(tBragg,braggAmpch1,braggCenter,braggWidth),zeros(size(tBragg)));     
    sq.dds(2).after(tBragg,110.-2-dopplerBragg*(timeSinceDrop+tBragg),sq.gauspulse(tBragg,braggAmpch2,braggCenter,braggWidth),zeros(size(tBragg)));    
