...
%     varargin{1};
    ...
    sq.dds(2).at(sq.ddsTrigDelay,110,0,0);
     ...
     RStable(:,end+1) = [5989456429.366; 19];
...
% 	sq.find('87 repump freq eom').after(tMicrowaveKnife,sq.expramp(tMicrowaveKnife,3.8,4.4,-3));
    sq.find('87 repump freq eom').after(tMicrowaveKnife,sq.expramp(tMicrowaveKnife,3.8,4.4,-3)); %end was 3.8,4.5
...
% 	sq.find('Vertical Bias').set(3.1);
%     sq.find('E/W Bias').set(0.7+0.5);
%     sq.find('N/S Bias').set(0.8);
    
    
    %% Raman Velocity Selection
    %Magnetic field
%	sq.find('E/W Bias').set(2.5);
% 	sq.find('N/S Bias').set(2.5);

    %Preparation
	sq.find('Bragg SSM Switch').set(1);
    sq.find('85 Repump Amp EOM').set(10);
    sq.find('87 Cooling Amp EOM').after(0.4e-3,0);
    sq.find('87 Cooling Freq EOM').after(0.2e-3,Freq2ToV(0,-60,'c'));
    sq.find('85 Repump TTL EOM').after(0.2e-3,1);
         
    %Timing  
    sq.anchor(droptime);
    tPulseDelay1 = 5e-3; %was 5
    sq.delay(tPulseDelay1);
    
	%Doppler Shift
    freq2hk = 0.0150837;
    dopplerBragg = tPulseDelay1*0.01255546;
     
    %Raman Light Frequencies
    
    
	%AOM offsets allowing counter-propagating 
    %Require:
        %offset1 - offset2 = TwoPhotonOffset 
        %or offset2 - offset1 = TwoPhotonOffset 
    offsetOnOff = 0;
    AOM1offset = offsetOnOff*7.5; %in MHz
    AOM2offset = offsetOnOff*2.5;
    TwoPhotonOffset = (AOM1offset - AOM2offset)*10^6
    
    %sideband detuning from carrier (i.e. 2 photon detuning)
	TwoPhotonDetuning = 0.5e5+6.834682611e9+0.02e5+varargin{2};
% 	TwoPhotonDetuning = varargin{2}+6.834682611e9+TwoPhotonOffset;
% 	TwoPhotonDetuning = 0.5e5+6.834682611e9+0.02e5+TwoPhotonOffset;
    RStable(:,end+1) = [TwoPhotonDetuning; 19];
    sq.find('RS Microwave TTL').set(1);
    sq.find('RS Microwave TTL').after(100e-6,0);   
    %allow carrier through 
    sq.find('SSB Carrier').set(1);
...
%     pulseWidth = varargin{2}
    pulseCenter = 400e-6;
    tBragg= linspace(0,800e-6,400);
    timeSinceDrop = sq.time -droptime;
    

    %pulse
    RamanDDS1Frequency = (110+0*freq2hk+AOM1offset)*ones(size(tBragg));
    RamanDDS1Amp = sq.gauspulse(tBragg,pulseAmpch1,pulseCenter,pulseWidth);
    sq.dds(1).after(tBragg,RamanDDS1Frequency,RamanDDS1Amp,zeros(size(tBragg)));
    
    RamanDDS2Frequency = 110.+AOM2offset-0-dopplerBragg*0*(timeSinceDrop+tBragg); %compensate for Doppler
    RamanDDS2Amp = sq.gauspulse(tBragg,pulseAmpch2,pulseCenter,pulseWidth);
    sq.dds(2).after(tBragg,RamanDDS2Frequency,RamanDDS2Amp,zeros(size(tBragg)));
...
% 	sq.find('E/W Bias').set(0);
%     sq.find('N/S Bias').set(0);
    
    
    EndRamanTime = sq.time;
    
    %% Imaging Delay
%     sq.anchor(sq.latest);
    sq.anchor(EndRamanTime);
	Tdrop = 1e-3;
%    Tdrop = varargin{1};
    sq.delay(Tdrop);
    TotalDropTime = ((EndRamanTime - droptime) + Tdrop)*10^3
    sq.find('Imaging AOM Amp').set(2.25);
        
% 	set field for imaging
    sq.find('Vertical Bias').before(1e-3,3); %was 0 for Raman, was 3 for no Raman
    sq.find('E/W Bias').before(1e-3,3);
% 	sq.find('Vertical Bias').before(1e-3,varargin{1}); %was 0 for Raman, was 3 for no Raman
%     sq.find('E/W Bias').before(1e-3,varargin{1});
    ...
    Timage=0.1*10^-3;
    sq.find('85 Repump TTL EOM').set(0);
    sq.find('Camera Trigger').set(1);
    sq.find('85 Repump Amp EOM').set(0);
    sq.find('87 Cooling Amp EOM').set(3);
    sq.delay(Timage);
    sq.find('Imaging AOM TTL').before(2*Timage,1);
    sq.find('Imaging AOM TTL').before(2*Timage,1);
    %after imagepulse settings
    sq.find('87 Cooling Freq EOM').set(Freq2ToV(0,-60,'c'));
    sq.find('Camera Trigger').set(0);
    sq.find('Imaging AOM TTL').set(0); %Note that this wasn't called in the repump pulse, hence you have to use both times
    sq.find('85 Repump Amp EOM').set(3);
    
    ImageDelay1=10e-3; %was 4ms
    sq.delay(ImageDelay1);
     
    %% Image F=1 & F=2 atoms
    imageVoltages2= Freq2ToV(0,0,'b'); %get both voltage, repump and cool
    RStable(:,end+1) = [double(imageVoltages2(1))*-1e6; 19];
    sq.find('87 Cooling Freq EOM').before(0.1*10^-3,imageVoltages2(2));
    sq.find('87 Cooling Amp EOM').before(0.1*10^-3,3);
    sq.find('85 Repump Amp EOM').before(0.1*10^-3,3);
    sq.find('RS Microwave TTL').set(1);
    sq.find('RS Microwave TTL').after(200e-6,0);
