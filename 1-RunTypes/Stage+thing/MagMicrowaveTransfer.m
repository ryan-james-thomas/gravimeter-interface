function varargout = makeSequence(varargin)
 ...
 RStable=double.empty(2,0);
    ...
    tdepump=3e-3;  %This was in the old run
...
%     sq.find('RS Microwave TTL').after(100e-6,0);   
%    tBragg= linspace(tBraggPulse,tBraggPulse+2000e-6,500);
%    braggAmpch2 = 0000;
%    braggAmpch1 = 3000;
%    braggWidth = 500e-6; 
%    braggCenter = tBraggPulse+1000e-6;
%    sq.dds(1).after(tBragg,(110-0)*ones(size(tBragg)),sq.gauspulse(tBragg,braggAmpch1,braggCenter,braggWidth),zeros(size(tBragg)));     
%    sq.dds(2).after(tBragg,110.+0*dopplerBragg*(timeSinceDrop+tBragg),sq.gauspulse(tBragg,braggAmpch2,braggCenter,braggWidth),zeros(size(tBragg)));    
