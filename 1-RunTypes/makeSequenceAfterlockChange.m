function varargout = makeSequence(varargin)
 ...
 repumpfreqs=double([]);
   ...
   repumpfreqs = [repumpfreqs double(Freq2ToV(repumpFrequency,coolingFrequency,'r'))];
...
%     sq.delay(5e-3);
%     %%Prepare for Bragg
%     sq.find('87 Cooling Amp EOM').set(0);
%     sq.find('87 Cooling Freq EOM').set(Freq2ToV(0,-60,'c'));
%     
...
%    timeSinceDrop = sq.time -droptime
 % tBraggPulse = 10e-3;
%    
%    repumpfreqs = [repumpfreqs 6834];
%    
  %  sq.find('RS Microwave TTL').after(tBraggPulse,1);
%     sq.find('SSB Carrier').after(tBraggPulse,1);
%     sq.find('Bragg SSM Switch').set(1);
  %  sq.find('RS Microwave TTL').after(100e-6,0);   
%    tBragg= linspace(tBraggPulse,tBraggPulse+800e-6,200);
%    braggAmpch2 = 7000;
%    braggAmpch1 = 7000;
%    braggWidth = 60e-6; 
%    braggCenter = tBraggPulse+400e-6;
%    
%    sq.dds(1).after(tBragg,(110+1+freq2hk)*ones(size(tBragg)),sq.gauspulse(tBragg,braggAmpch1,braggCenter,braggWidth),zeros(size(tBragg)));     
%    sq.dds(2).after(tBragg,110.-1-1.*dopplerBragg*(timeSinceDrop+tBragg),sq.gauspulse(tBragg,braggAmpch2,braggCenter,braggWidth),zeros(size(tBragg)));    
...
%     sq.delay(tFMI);
    ...
    repumpfreqs = [repumpfreqs double(imageVoltages(1))];
     sq.find('Repump/Microwave Switch').set(0);
...
%    sq.find('SSB Carrier').set(0);
     sq.find('85 Repump Amp EOM').before(0.1*10^-3,10);
...
%    %after imagepulse settings
     ...
     sq.find('85 Repump Amp EOM').after(Timage,2.1);
...
%    %after imagepulse settings
     ...
     sq.find('Camera Trigger').after(Timage,0);
     sq.find('Imaging AOM TTL').after(Trepump+Timage,0); %Note that this wasn't called in the repump pulse, hence you have to use both times
     sq.find('85 Repump Amp EOM').after(Timage,10);
