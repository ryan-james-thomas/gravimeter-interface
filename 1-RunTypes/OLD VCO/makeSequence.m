function varargout = makeSequence(varargin)
    ...
    MagRampStepSize=10e-3;
    MicrowaveStepSize=20e-3;
    DipoleStepSize=10e-3;
    tMagTrap=4;
    tMagRampDown=0.5;
    tMicrowave2=0.5;
    ...
    sq.find('WG AMP 3').after(tDipoleRamp,sq.linramp(tDipoleRamp,0.0,0));
...
%     sq.find('87 Cooling Freq EOM').set(0);
%     
%     powerRatio=3500/4000;
%     
%     freq2hk = 0.0150837;
%     doppler = 1.55*10^3*0.01255546; %MHz/s 
%    
%     
%     %bloch ramp up
%     pulseTime= 3e-3;
...
%     finishFreq = startFreq-30.*freq2hk;
%     
%     pulseAmpch2=4000;
...
% 
%     
%     tBlochOn=linspace(pulseTime,pulseTime+tBlochRamp,tBlochRamp/BlochStepSize);
%     tBlochS=linspace(pulseTime+tBlochRamp,pulseTime+tBlochRamp+tBlochSweep,tBlochSweep/BlochStepSize);
%     tBlochOff=linspace(pulseTime+tBlochRamp+tBlochSweep,pulseTime+tBlochRamp+tBlochSweep+tBlochRamp,tBlochRamp/BlochStepSize);
%     
...
%         [sq.linramp(tBlochOn,sq.dds(1).values(end,2),pulseAmpch1) pulseAmpch1*ones(size(tBlochS)) sq.linramp(tBlochOff,pulseAmpch1,0)],...
...
%         zeros(size([tBlochOn tBlochS tBlochOff])));
%     
% %     sq.find('3DHMOT Amp AOM').after([tBlochOn tBlochS tBlochOff],sq.minjerk([tBlochOn tBlochS tBlochOff],sq.find('3DHMot Amp AOM').values(end),-0.46));
% 
%     sq.dds(1).after(4e-6,110,0.,0);
%     sq.dds(2).after(4e-6,110,0.,0);
% %     
% 
%    
%     
...
%     tBlochRamp = 100e-6;
%     tBlochSweep=700e-6;
%     startFreq=110-30*freq2hk;
%     finishFreq = startFreq-10.*freq2hk;
%     doppler = 1.55*10^3*0.01255546;
...
%         zeros(size([tBlochOn tBlochS tBlochOff])));
    ...
    sq.dds(1).after(10e-6,110,0.,0);
    sq.dds(2).after(10e-6,110,0.,0);
...
%         sq.anchor(sq.latest);
%     
%     dopplerBragg = 1.2*10^3*0.01255546;
%    timeSinceDrop = sq.time -droptime;
%    tBraggPulse = 2e-3;
%    tBragg= linspace(tBraggPulse,tBraggPulse+400e-6,200);
%    braggAmpch2 = 40;
%    braggAmpch1 = 40;
%    braggWidth = 25e-6; 
...
%     sq.find('E/W Bias').before(1e-3,3);      %set the fields needed for image
    ...
    sq.find('WG 1 TTL').before(1e-3,0);
    sq.find('WG AMP 1').before(1e-3,0);
