function varargout = makeSequence(varargin)
    ...
    sq.find('87 repump freq eom').after(tMicrowaveKnife,sq.expramp(tMicrowaveKnife,4.3,4.73,-3));
...
%     sq.find('WG AMP 1').after(tDipoleRamp,sq.linramp(tDipoleRamp,0.2,0));
    ...
    sq.find('87 repump freq eom').after(tMicrowaveKnife2,sq.expramp(tMicrowaveKnife2,4.73,4.77,-3));
...
%         %bloch ramp up sq.find('FMI Trigger').after(pulseTime,1);
   ...
   tBraggPulse =0.1e-3;
   tBragg= linspace(tBraggPulse,tBraggPulse+200e-6,100);
   braggAmpch2 = 7000;
