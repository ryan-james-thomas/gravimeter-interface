function BiasScanMeasureTemp(r)

if r.isInit()
    %Initialize run
    r.data.tof = (4:4:12)*10^-3; %this is varargin{1}
    r.data.param = (4.3:0.03:4.6); %this varargin{2}
    r.data.tRaman = 5.9e-3;
...
%     r.data.Ty(i1,i2) = c.T(2);
    ...
    subplot(1,2,1);
    cla;
    plot(r.data.tof(1:i1)+r.data.tRaman,r.data.yw(1:i1,i2),'o-');
    hold on;
    plot(r.data.tof(1:i1)+r.data.tRaman,r.data.xw(1:i1,i2),'o-');
    hold off;
    subplot(1,2,2)
    
    if r.data.count.idx(1) == r.data.count.maxRuns(1)
        %cla;
        lf = linfit(r.data.tof+r.data.tRaman,r.data.yw(:,i2).^2,2*r.data.yw(:,i2).*10e-6);
        ...
        lf.set(r.data.tof+r.data.tRaman,r.data.xw(:,i2).^2,2*r.data.xw(:,i2).*10e-6);
        lf.fit;
        lf.plot;
        r.data.Tx(i2,1) = lf.c(2,1)*const.mRb/const.kb;
        figure(10);
        subplot(1,2,2);
        plot(r.data.param(1:i2),r.data.Ty(1:i2),'o-');
        hold on
        plot(r.data.param(1:i2),r.data.Tx(1:i2),'o-');
        hold off;
       % ylim([0,100e-6]);
    end
