...
elseif r.isSet()
    ...
    r.make(r.data.param(r.currentRun),20e-3);
    r.upload;
...
elseif r.isAnalyze()
    ...
    figure(10);clf;
    errorbar(r.data.param(1:nn),r.data.xw,2e-6*ones(size(r.data.xw)),'o');
    hold on;
    errorbar(r.data.param(1:nn),r.data.yw,2e-6*ones(size(r.data.yw)),'sq');
